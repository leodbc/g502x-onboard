...
# Modified in G502 X Onboard for validated transport/loading behavior.
...
# Keep this handle alive for the lifetime of the module. On CPython 3.8+
# closing/collecting the object returned by os.add_dll_directory removes that
# directory from the DLL search path used by ctypes.
_HIDAPI_DLL_DIRECTORY_HANDLE = prepare_windows_hidapi_directory(__file__)

# hidapi native binary:
# https://github.com/libusb/hidapi
#
# pyhidapi ctypes binding:
...
try:
    ...
    if sys.platform == "win32":
        detail = (
            "Unable to load the bundled hidapi.dll through pyhidapi. "
            "Use the release-matched libs/x64 or libs/x86 directory and a "
            "supported Python architecture."
        )
    elif sys.platform.startswith("linux"):
        detail = (
            "Unable to load native hidapi through pyhidapi. Install a native "
            "hidapi runtime such as libhidapi-hidraw0, then retry."
        )
    else:
        detail = "Unable to load a native hidapi runtime through pyhidapi."
    raise ImportError(detail) from exc

verify_windows_hidapi_loaded(__file__, hid)
    ...
    def __init__(self, pid = 0, name = '', serial = '', index_list = []):
        ...
        for dev in devs:
            ...
            if dev['usage_page'] >= 0xFF00:
                ...
                data = bytes(h.get_report_descriptor())
